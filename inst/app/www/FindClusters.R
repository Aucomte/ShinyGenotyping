...
## find.clusters2 
#############
find.clusters2  <- function (x, ...) UseMethod("find.clusters")

############################
## find.clusters.data.frame2
############################
find.clusters.data.frame2 <- function(x, clust = NULL, n.pca = NULL, n.clust = NULL,
...
########################
## find.clusters.genind2
########################
find.clusters.genind2 <- function(x, clust = NULL, n.pca = NULL, n.clust = NULL, 
                                 method = c("kmeans", "ward"),
                                 stat = c("BIC", "AIC", "WSS"),
                                 choose.n.clust=TRUE, 
  ...
  res <- find.clusters2 (X, clust=clust, n.pca=n.pca, n.clust=n.clust, stat=stat,
...
###################
## find.clusters.matrix2
###################
find.clusters.matrix2 <- function(x, ...){
  return(find.clusters2 (as.data.frame(x), ...))
}
...
##########################
## find.clusters.genlight2
##########################
find.clusters.genlight2 <- function(x, clust = NULL, n.pca = NULL, n.clust = NULL,
                                   method = c("kmeans", "ward"),
                                   stat = c("BIC", "AIC", "WSS"),
                                   choose.n.clust = TRUE, 
                                   criterion = c("diffNgroup", "min","goesup", "smoothNgoesup", "goodfit"),
...
.find.sub.clusters <- function(x, ...){
  ...
  for(i in levels(clust)){
    ...
    temp <- do.call(find.clusters2 , myArgs)$grp
